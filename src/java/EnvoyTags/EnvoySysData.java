 ...
 * @author cmarshal
 */
public class EnvoySysData extends BodyTagSupport {
    private String envoyHost;
    private ArrayList envoysysdata;
    private ArrayList envoybadsysdata;
    private int index = 0;
    ...
    private void otherDoStartTagOperations() {
        envoysysdata = new ArrayList();
        envoybadsysdata = new ArrayList();
        ...
        try {
            ...
            for (Element h2 : allh2) {
                if (h2.text().equals("System Operation")) {
                    Elements alldiv = h2.parent().getElementsByTag("div");
                    for (Element div : alldiv) {
                        if (div.text().length() > 0) {
                            if (div.attr("class").equals("bad")) {
                                envoybadsysdata.add("true");
                            } else {
                                envoybadsysdata.add("false");
                            }
                            envoysysdata.add(div.text());
                        }
                    }
                }
            }
    ...
    private void setOutput() {
        if (envoybadsysdata.size() > 0) {
            pageContext.setAttribute("envoybadsysdata", envoybadsysdata.get(index));
            pageContext.setAttribute("envoysysdata", envoysysdata.get(index));
        }
    }
